...
// 食谱库：外部 GitHub 仓库定期同步的只读内容，无用户维度——全家共享；
// 多数据源（source 区分），分类 = 源内目录名映射，菜名 = md 文件名
export const recipes = sqliteTable(
  "recipes",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    source: text("source").notNull().default("cooklikehoc"), // 数据源：cooklikehoc | howtocook
  ...
  (t) => [uniqueIndex("idx_recipes_source_dish").on(t.source, t.sourcePath), index("idx_recipes_category").on(t.category)]
);

// 食谱同步状态（每数据源一行）：记录上游 commit 与最近一次同步结果
export const recipeSyncState = sqliteTable(
  "recipe_sync_state",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    source: text("source").notNull().default("cooklikehoc"),
    lastCommit: text("last_commit").notNull().default(""), // 上游默认分支最新 commit sha
