...
其他 agent（Claude Code / Cursor 等）通过 MCP 接入 Oak，调用与 Web 端 AI 助手一致的 **20 个只读查询工具**（成员/教师/食谱/健身馆/成长/健康/账单/学习/时光/提醒/待办/笔记/园地/掌握度/卡证/政策/快记/洞察/跨模块搜索/记忆检索 searchKnowledge）。所有工具按令牌归属账号在库中隔离；**不含联网搜索与网页抓取**（避免烧钱/SSRF）。
