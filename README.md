...
- **AI 助手（悬浮对话）**：右下角悬浮入口的全家数据管家，AI SDK v7 流式对话 + 19 个只读查询工具（成员/教师/食谱/成长/健康/账单/学习/时光/提醒/待办/笔记/学习园地/卡证/政策/快记/洞察 + 跨模块搜索 + 记忆检索），回答前强制查库不编造；多模型配置（设置页：DeepSeek / OpenAI / Kimi / 通义 / 自定义，优先 Responses API 接口形态），可选联网搜索（AnySearch 或 DeepSeek 原生 web_search）；会话与消息入库可回溯
...
- **食谱**：《像老乡鸡那样做饭》+《程序员做饭指南》双源自动同步（GitHub 定期拉取、commit 比对免重复下载、图片本地化、上游增删自动跟随），20 个分类 700+ 道菜按分类浏览、按菜名/食材/步骤全文搜索，卡片与详情带来源标记；「今天吃什么」一键让 AI 按当前餐段搭配 2-3 道菜并给理由，AI 未配置时降级随机推荐；AI 助手/MCP 可用 `queryRecipes` 工具检索菜谱
- **MCP 接入（AI 开放接口）**：设置页生成访问令牌后，其他 agent 可经 stdio（`OAK_TOKEN=<令牌> npx tsx mcp/server.ts`）或 HTTP（`/api/mcp`，Bearer 令牌）调用与 AI 助手一致的 19 个只读查询工具，按令牌账号隔离数据；不含联网搜索，撤销/过期即时生效
