...
- **MCP 接入（AI 开放接口）**：设置页生成访问令牌后，其他 agent 可经 stdio（`OAK_TOKEN=<令牌> npx tsx mcp/server.ts`）或 HTTP（`/api/mcp`，Bearer 令牌）调用与 AI 助手一致的 18 个只读查询工具，按令牌账号隔离数据；不含联网搜索，撤销/过期即时生效
