...
# 竹子的三根竿：叶 / 穗 / 颖果全部用 foliage_on() / heads_on() 按竿轴参数 t 求挂点，
# 不再手写会随斜竿方位漂移的坐标。竿 0 是主竿——elev=0、az=0、bend 与 stem.bend 相同，
# 于是 along(竿 0, t) = (bend·t^1.7, 0, H·t) 正好落在主竿轴线上（主竿几何仍由 stem 的
# culm() 生成，这一项只用来算挂点）。竿 1/2 就是 stem.branches 里的两根斜竿。
_BAMBOO_H = 1.16
_BAMBOO_BEND = 0.020          # 主竿向 +x 的弧弯；竿 0 的 bend 必须与它一致
_BAMBOO_CULMS = [
    limb(length=_BAMBOO_H, r=0.020, segs=8, elev=0.0, az=0.0, bend=_BAMBOO_BEND),
...
# 叶：15 片分 5 簇（竿 0 两簇 z≈1.06/0.94，竿 1 一簇，竿 2 两簇）。
# picks = [(竿号, t, 方位偏移°, 上抬角°, 缩放)]，方位偏移 = 目标方位 − 竿的方位。
_BAMBOO_LEAF_PICKS = [
    (0, 0.914, 20.0, 18.0, 1.05), (0, 0.914, 95.0, 26.0, 0.98),
    (0, 0.914, 170.0, 12.0, 0.92), (0, 0.914, 245.0, 22.0, 0.88),
    (0, 0.810, 215.0, 16.0, 1.00), (0, 0.810, 290.0, 28.0, 0.94),
    (0, 0.810, 350.0, 8.0, 0.86), (0, 0.810, 60.0, 24.0, 0.90),
    (1, 0.88, -10.0, 20.0, 0.96), (1, 0.88, 60.0, 10.0, 0.88),
    (1, 0.88, 125.0, 30.0, 0.82),
    (2, 0.91, -15.0, 24.0, 0.92), (2, 0.91, 55.0, 14.0, 0.86),
    (2, 0.76, 120.0, 20.0, 0.88), (2, 0.76, 235.0, 28.0, 0.80),
]
# 花：三竿各一小穗（竿 0 顶 z≈1.13、竿 1 的 z≈0.92、竿 2 的 z≈0.78）。
# picks = [(竿号, t, 仰角偏移°, 缩放)]，仰角 = 竿的仰角 + 偏移（主竿的穗本来就朝上，用 8°）。
_BAMBOO_FLOWER = heads_on(_BAMBOO_CULMS, _BAMBOO_H, [
    (0, 0.974, 8.0, 1.00), (1, 0.94, 11.0, 0.90), (2, 0.95, 5.5, 0.80)],
    core=0.007, core_h=0.016, core_segs=5,
...
# 颖果：3 粒米粒状小果，与三小穗同点（dz 同样相对竿顶）
_BAMBOO_BERRIES = [dict(x=h["x"], y=h["y"], dz=h["dz"], r=r)
                   for h, r in zip(_BAMBOO_FLOWER["heads"], (0.008, 0.007, 0.006))]
...
SPECIES = {
    ...
    "bamboo": {
        ...
        # （斜竿用 _BAMBOO_CULMS[1:]，与叶/穗/果的挂点共用同一批 limb()）
        "stem": dict(color=(0.46, 0.60, 0.32), bend=_BAMBOO_BEND,
                     **culm(h=_BAMBOO_H, r=0.020, segs=8,
                            nodes=[0.15, 0.30, 0.45, 0.60, 0.75, 0.90]),
                     branches=_BAMBOO_CULMS[1:]),
        # 叶：15 片窄披针形，成 5 簇挂在三竿的中上部，叶尖明显下垂（竹叶的标志）；
        # 挂点由 foliage_on() 按竿轴求，斜竿上的叶不会再被手写坐标甩到竿外
        "foliage": foliage_on(_BAMBOO_CULMS, _BAMBOO_LEAF_PICKS,
                              color=(0.34, 0.55, 0.27), shape=NARROW_W,
                              blade=0.155, wid=0.023, petiole=0.020, petiole_r=0.0035,
                              petiole_segs=5, droop=0.60, fold=0.18, twist=0.60,
                              thick=0.005, nu=5, nv=4),
        # 花：细小穗状——三竿各一小穗（4 片淡黄绿小瓣 + 小米粒花心），挂点由 heads_on() 求
        "flower": _BAMBOO_FLOWER,
        # 果：3 粒极小颖果（米粒状），与三小穗同点
        "fruit": dict(color=(0.72, 0.66, 0.42), berry_segs=6,
                      berries=_BAMBOO_BERRIES),
    },
